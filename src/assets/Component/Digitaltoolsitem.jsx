...
const Digitaltoolsitem = ({ mainData, selectedItems, setSelectedItems }) => {
    ...
    const btnHendle = (value, data) => {
        setTogolingBtn(value);
        toast(data) 
    }
    return (
        <div className='w-10/12 mx-auto py-[85px] space-y-10'>
            <header className='text-center space-y-4'>
                ...
                <div className='flex justify-center gap-2 shadow-blue-300 shadow w-[280px] mx-auto rounded-full p-1'>
                    <button onClick={() => btnHendle('Products', '🖐️ You clicked Products btn!')} className={`cursor-pointer hover:scale-105 duration-300 ${togolingBtn === 'Products' ? 'bg-linear-500 from-[#4F39F6] to-[#9514FA] text-white' : 'bg-gray-200 text-gray-700'} py-3 px-8 rounded-full font-bold cursor-pointer`}>Products</button>
                    <button onClick={() => btnHendle('Cart', '🖐️ You clicked Cart btn!')} className={`cursor-pointer hover:scale-105 duration-300 ${togolingBtn === 'Cart' ? 'bg-linear-500 from-[#4F39F6] to-[#9514FA] text-white' : 'bg-gray-200 text-gray-700'} py-3 px-8 rounded-full font-bold cursor-pointer`}>Cart ({selectedItems.length})</button>
                </div>
            </header>
            <main>
                {
